...
QByteArray& QPScriptTokenList::toString() {
...
QByteArray& QPScriptTokenStack::popType(const QByteArray &requestedType) const {
	QPScriptTokenPtr t = mStack.back();
	mStack.pop_back();
	
	if (qstrcmp(requestedType.data(), t->metaObject()->className()))
		return t;
	else
		throw QPScriptError("Expected " + requestedType.data() + " element. Got " t->metaObject()->className() + " element instead.");
}
